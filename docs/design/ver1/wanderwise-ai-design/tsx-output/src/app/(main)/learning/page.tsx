...
import {
  ...
  File, MessageSquare, Info, RefreshCw, Plus,
} from 'lucide-react'

// ---------------------------------------------------------------
// ベクトル可視化（静的モック — /api/learning/visualize で置き換え可）
// ---------------------------------------------------------------
...
function StatusBadge({ status }: { status: Document['status'] }) {
  ...
  return (
    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${className} flex items-center gap-1`}>
      {status === 'processing' && (
...
export default function LearningPage() {
  ...
  const totalChunks  = docs.filter((d) => d.is_active && d.status === 'vectorized')
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-600">
                        ...
                        {doc.size} · {new Date(doc.updated_at).toLocaleDateString('ja-JP')}
                        {doc.chunks != null && (
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm rounded-xl transition-all shadow-[0_0_16px_rgba(37,99,235,0.3)]"
              >
                検索
              </button>
            </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {results.map((r, i) => (
                    <div
                      key={i}
                      className="p-4 rounded-2xl bg-white/[0.02] border border-white/8 hover:border-white/14 transition-colors"
                    >
